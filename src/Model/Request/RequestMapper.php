...
class RequestMapper
{
    ...
    private static function getProduct($request)
    {
        ...
        return ObjectFactory::create(Product::class, [
            'productCode'       => $code,
            'homeDelivery'      => null,
    ...
    private static function getCustoms($request)
    {
        ...
        foreach ($request['package_items'] AS $item) {
            $i++;
            $lines[] = ObjectFactory::create(CustomsLine::class, [
                // description of the content
                'description' => $item['name'] ? $item['name'] : $request['params']['customs']['export_description'],
                // customs tarif number
                'harmonizedSystemCode' => (isset($item['customs_item_hs_code']) ? substr($item['customs_item_hs_code'], 0, 8) : ""),
        ...
        return ObjectFactory::create(Customs::class, [
...
//
     ...
     * @return mixed
     */
    public static function mapShipmentOrderRequest(array $shipmentOrders, $async = true, $url = false)
    {
